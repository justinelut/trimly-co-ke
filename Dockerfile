...
ARG NEXT_PUBLIC_WEBAPP_URL=http://NEXT_PUBLIC_WEBAPP_URL_PLACEHOLDER

## Trimly analytics + SEO. NEXT_PUBLIC_* vars are inlined into the bundle at
## build time, so they MUST be passed as build-args here (not just set at
## runtime in the K8s secret) or @next/third-parties will boot with empty IDs.
ARG NEXT_PUBLIC_GA_ID
ARG NEXT_PUBLIC_GTM_ID
ARG NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION
ARG TRIMLY_WHATSAPP_NUMBER
...
ENV NEXT_PUBLIC_WEBAPP_URL=$NEXT_PUBLIC_WEBAPP_URL \
  ...
  TRIMLY_WHATSAPP_NUMBER=$TRIMLY_WHATSAPP_NUMBER \
